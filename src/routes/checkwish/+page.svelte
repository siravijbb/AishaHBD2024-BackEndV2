	...
	{#if form?.wish}
		...
		{#each form?.wish as rom}
			<form 						on:submit|preventDefault={handleSubmit}
							 ...
							 action="?/approve"
								>
			<p class="hidden">ID:<input class="p-2 rounded-lg text-sm bg-gray-500 "   type="text" id="id" name="id" value={rom.id}></p>
			<p>NAME: {rom.name}</p>
			<p class="text-sm">WISH: {rom.comment}</p>

				<button class="bg-green-500 rounded-lg mt-2 p-2 hover:bg-green-500/70" 									disabled={loading} formaction="?/approve">Approve</button>
				<button class="bg-red-500 rounded-lg mt-2 p-2 hover:bg-red-500/70" 									disabled={loading} formaction="?/notapp">SPAM</button>
